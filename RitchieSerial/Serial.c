 ...
 */
	...
	struct RingBuffer rx_buffer  =  { { 0 }, 0, 0 };
	void Serial(struct Serial * const serial) {
		SerialInit(serial, &rx_buffer, &UBRRH, &UBRRL, &UCSRA, &UCSRB, &UDR, RXEN, TXEN, RXCIE, UDRE, U2X);
	}
#elif defined(UBRR0H) && defined(UBRR0L)
	struct RingBuffer rx_buffer  =  { { 0 }, 0, 0 };
	void Serial(struct Serial * const serial) {
		SerialInit(serial, &rx_buffer, &UBRR0H, &UBRR0L, &UCSR0A, &UCSR0B, &UDR0, RXEN0, TXEN0, RXCIE0, UDRE0, U2X0);
	}
#elif defined(USBCON)
	#include "usb_api.h"
	...
	void Serial(struct Serial * const serial) {
		SerialInit(serial, &rx_buffer1, &UBRR1H, &UBRR1L, &UCSR1A, &UCSR1B, &UDR1, RXEN1, TXEN1, RXCIE1, UDRE1, U2X1);
	}
#endif
#if defined(UBRR2H)
	ring_buffer rx_buffer2  =  { { 0 }, 0, 0 };
	void Serial(struct Serial * const serial) {
		SerialInit(serial, &rx_buffer2, &UBRR2H, &UBRR2L, &UCSR2A, &UCSR2B, &UDR2, RXEN2, TXEN2, RXCIE2, UDRE2, U2X2);
	}
#endif
#if defined(UBRR3H)
	ring_buffer rx_buffer3  =  { { 0 }, 0, 0 };
	void Serial(struct Serial * const serial) {
		SerialInit(serial, &rx_buffer3, &UBRR3H, &UBRR3L, &UCSR3A, &UCSR3B, &UDR3, RXEN3, TXEN3, RXCIE3, UDRE3, U2X3);
	}
#endif
